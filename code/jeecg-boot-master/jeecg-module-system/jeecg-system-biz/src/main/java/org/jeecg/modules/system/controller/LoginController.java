...
//		}
	...
	public Result<String> sms(@RequestBody JSONObject jsonObject) {
		...
		//60s 如果有不能重复发送验证码信息
		String mobile60s = jsonObject.get("mobile").toString()+"60s";
		//手机号模式 登录模式: "2"  注册模式: "1"
		...
		//判断验证码在redis中存放够不够一分钟  不够一分钟,不能调用sendSms,如果超过一分钟调用sendSms
		Object object = redisUtil.get(mobile60s);
		if (object != null) {
			result.setMessage("一分钟之内不能重复发送验证码");
			result.setSuccess(false);
			return result;
		}
		...
		try {
			...
			if (CommonConstant.SMS_TPL_TYPE_1.equals(smsmode)) {
				...
				b = DySmsHelper2.sendSms(mobile, captcha);
			}else {
				//登录模式，校验用户有效性
				SysUser sysUser = sysUserService.getUserByPhone(mobile);
				result = sysUserService.checkUserIsEffective(sysUser);
			...
			redisUtil.set(mobile60s,captcha, 60);
